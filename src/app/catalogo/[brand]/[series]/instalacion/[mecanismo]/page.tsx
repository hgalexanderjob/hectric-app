...
export default async function InstalacionMecanismoPage({
  ...
  const mecanismoData = standardMechanisms.find(
    (m) => m.id.toLowerCase() === mecanismoSlug.toLowerCase()
  );
          ...
          <div className="lg:col-span-7 xl:col-span-8 flex flex-col gap-y-6">
            ...
            <div>
              <div className="flex items-center gap-x-3 mb-2">
                <span className="inline-flex items-center py-1 px-2.5 rounded-md text-xs font-semibold bg-teal-100 text-teal-800">
                  {mecanismoData.name}
                </span>
                <span className="text-sm font-medium text-slate-500">
                  Serie {seriesData.name}
                </span>
              </div>
            ...
            <div className="relative w-full overflow-hidden rounded-2xl shadow-xl border border-slate-200 bg-black" style={{ paddingTop: "56.25%" }}>
              <iframe
                ...
                title={`Tutorial de instalación ${mecanismoData.name} ${seriesData.name}`}
            ...
            {/* Comunidad y Consejos (Pasando también el mecanismo) */}
            <CommunityComments brandName={brandData.name} seriesName={seriesData.name} mecanismoName={mecanismoData.name} />
          ...
          <div className="lg:col-span-5 xl:col-span-4 h-full">
            <div className="sticky top-[140px]">
              <AIChat 
                brandId={brandSlug} 
                seriesId={seriesSlug} 
                mecanismoId={mecanismoSlug} 
                seriesName={`${brandData.name} ${seriesData.name}`} 
                mecanismoName={mecanismoData.name} 
              />
            </div>
          </div>
