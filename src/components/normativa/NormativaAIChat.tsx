...
export default function NormativaAIChat() {
  ...
  const initialMsgs = useMemo(() => ([
    {
      ...
      content: `¡Hola! Soy hectric IA, tu asistente experto en el **Reglamento Electrotécnico para Baja Tensión (REBT)** y sus Instrucciones Técnicas Complementarias (ITC-BT). 

¿Tienes alguna duda sobre secciones de cables, protecciones, locales de pública concurrencia o cálculos eléctricos? ¡Pregúntame!`,
    }
  ]), []);
      ...
      <div className="p-4 bg-white/80 backdrop-blur-md border-t border-slate-200/60">
        <form onSubmit={handleSubmit} className="relative flex items-center group">
          <input
            ...
            disabled={isLoading}
            placeholder="Pregunta sobre la normativa..."
            className="w-full pl-4 pr-12 py-3.5 bg-slate-50/50 border border-slate-200/80 rounded-lg text-sm text-slate-700 placeholder:text-slate-400 focus:border-teal-500/50 focus:bg-white focus:ring-4 focus:ring-teal-500/10 transition-all disabled:opacity-50 outline-hidden shadow-xs"
          />
  ...
  return (
    <>
      ...
      <button 
        ...
        aria-label="Abrir Asistente de Normativa"
      >
        <div className="relative">
          <div className="size-14 rounded-lg bg-teal-50/90 flex items-center justify-center shadow-[0_8px_30px_rgb(0,0,0,0.15)] border border-teal-100/80 overflow-hidden backdrop-blur-sm">
